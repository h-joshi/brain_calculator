...
2. Apply the SQL files in `supabase/migrations/` in numeric order using the SQL editor (or `supabase db push`). For a project that already has multiplayer installed, apply only migrations it has not run; do not rerun `001_multiplayer.sql`. `002_rounds_and_games.sql` adds the configurable rounds-and-games match format.
